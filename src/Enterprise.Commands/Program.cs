...
using Enterprise.Commands.Data;
...
Log.Logger = new LoggerConfiguration()
    .WriteTo.Console()
    .WriteTo.File("logs/commands-.txt", rollingInterval: RollingInterval.Day)
    .CreateLogger();

try
{
    var rootCommand = new RootCommand("Enterprise Commands - CLI tool for database operations");
...
{
    Log.Fatal(ex, "An error occurred in the commands application");
    return 1;
}
finally
{
